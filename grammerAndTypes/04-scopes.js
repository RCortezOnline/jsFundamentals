...
- Scope is how a comnputer keeps track of all of it's variables in a program.
- Scope can be nested, where ther eis an outer scope that encloses an inner scope

- Global:  Variables that are defined outside of a function.  
Functions can have acess to variables that are within the global scope.  Consider 
global scope just like we would consider Earth in relation to Indiana.  
There are a lot of "things" that could be happening outside of our local environment 
that could affect our state and a lot of "things" that wouldn't.
...
if(true) {
    ...
    Var and Let seem to operate the same way, but while do have a lot of the same functionality (both let us declare and initialize variables, the behave differently)
